...
async fn test_create_order_flow() {
    ...
    // 1. Signup & Login
    let email = format!("test_order_{}@example.com", Uuid::new_v4());
    ...
    let order_response: OrderResponse = serde_json::from_slice(&order_body).unwrap();
    
    assert_eq!(order_response.status, "PAID");
}
